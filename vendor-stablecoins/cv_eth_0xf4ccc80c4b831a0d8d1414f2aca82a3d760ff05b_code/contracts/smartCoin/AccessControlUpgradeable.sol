    ...
    Initializable
{
    ...
    /**
     * @dev Used when a method reserved to the allowed minters is called by some other address
     */
    error UnauthorizedMinter();
    /**
     * @dev Used when a method reserved to the allowed burner is called by some other address
     */
    ...
    /**
     * @dev Address is already a redemption address 
     */
    error AlreadyRedemptionAddress(address addr);

    /**
     * @dev Address is not a redemption address
     */
    error NotRedemptionAddress(address addr);
    ...
    function findNotFrozen(
        ...
        address[] memory notFrozenAddresses = new address[](
            getNumberOfNotFrozenAddresses(addrs)
        );
        uint16 count = 0;
    ...
    /**
     * @dev add redemption addresses
     * NB: only the (current) registrar operator can call this method
     */
    function addRedemptionAddresses(
        address[] calldata addrs
    ) external onlyRegistrar returns (bool) {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (isRedemptionAddress[addrs[i]])
                revert AlreadyRedemptionAddress(addrs[i]);
            isRedemptionAddress[addrs[i]] = true;
        }
        emit RedemptionAddressesAdded(addrs);
        return true;
    }
    /**
     * @dev remove redemption addresses
     * NB: only the (current) registrar operator can call this method
     */
    function removeRedemptionAddresses(
        address[] calldata addrs
    ) external onlyRegistrar returns (bool) {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (!isRedemptionAddress[addrs[i]])
                revert NotRedemptionAddress(addrs[i]);
            isRedemptionAddress[addrs[i]] = false;
        }
        emit RedemptionAddressesRemoved(addrs);
        return true;
    }
}
