 ...
 * Transfers to the registrar operator or the operations operator will occur
 * once validated by the registrar operator using the validateTransfer method
 */
interface ISmartCoin {
    /**
     * @dev Emitted when a transfer to redemption address is processed
     */
    ...
    /**
     * @dev Transfer the balance of `_addr` to registrar address
     * The tokens are transferred then burned
     *
     * NB: This method is reserved to the registrar operator.
     */
    function wipeFrozenAddress(address _addr) external returns (bool);
